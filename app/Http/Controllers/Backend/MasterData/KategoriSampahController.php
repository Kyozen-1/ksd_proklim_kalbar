...
class KategoriSampahController extends Controller
{
    ...
    {
        return view('backend.master-data.kategori-sampah.index');
    }

    public function datatable()
    {
        $getData = new MdKategoriSampah;
    ...
    public function store(Request $request)
    {
        ...
        try {
            $kategoriSampah = new MdKategoriSampah;
            $kategoriSampah->user_id = Auth::user()->id;
            $kategoriSampah->nama = $request->nama;
            $kategoriSampah->status_aktif = '1';
            $kategoriSampah->save();

            return response()->json(['success' => 'Berhasil menambahkan Kategori Sampah']);
        } catch (\Throwable $th) {
    ...
    public function show(string $id)
    {
        ...
        $getData = MdKategoriSampah::find($id);
        $dataSend = [
            'nama' => $getData->nama
        ];
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $kategoriSampah = MdKategoriSampah::find($id);
            $kategoriSampah->nama = $request->nama;
            $kategoriSampah->save();

            return response()->json(['success' => 'Berhasil merubah Kategori Sampah']);
        } catch (\Throwable $th) {
            return response()->json(['errors' => $th->getMessage()]);
        }
    }
    ...
    public function destroy(string $id)
    {
        try {
            $id = Crypt::decryptString($id);
            $kategoriSampah = MdKategoriSampah::find($id);
            $kategoriSampah->status_aktif = '0';
            $kategoriSampah->save();

            return response()->json(['success' => 'Berhasil menghapus Kategori Sampah']);
        } catch (\Throwable $th) {
            return response()->json(['errors' => $th->getMessage()]);
        }
    }
