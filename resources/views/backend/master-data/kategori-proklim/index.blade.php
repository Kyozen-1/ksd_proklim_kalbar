...
@section('title', 'Kategori Proklim | Master Data | PROKLIM Kalimantan Barat')
@section('header', 'Kategori Proklim | Master Data')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card-box table-responsive">
                ...
                <table id="table_md_kategori_proklim" class="table table-bordered table-bordered dt-responsive nowrap">
                    <thead>
                        <tr>
                            <th width="5%">No</th>
                            <th>Nama</th>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="form_md_kategori_proklim" method="POST" data-parsley-validate novalidate>
                        @csrf
                        <div class="form-group">
                            ...
                            placeholder="Masukan nama Kategori..." class="form-control">
                        </div>
                </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group row">
                        <label for="detail_nama" class="control-label col-md-6">Nama Kategori</label>
                        <div class="col-md-6">
...
@section('js')
    ...
    <script>
        var dataTables = $('#table_md_kategori_proklim').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('cms.master-data.kategori-proklim.datatable') }}",
            },
            columns:[
                {
                    data: 'DT_RowIndex'
        ...
        $('#create').click(function(){
            $('#form_md_kategori_proklim')[0].reset();
        ...
        $(document).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.master-data.kategori-proklim.show', ['id' => ":id"]) }}";
        ...
        $('#form_md_kategori_proklim').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('cms.master-data.kategori-proklim.store') }}",
                    method: "POST",
                    data: $(this).serialize(),
                    dataType: "json",
                    beforeSend: function()
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.errors)
                        {
                            ...
                            $('#form_md_kategori_proklim')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#table_md_kategori_proklim').DataTable().ajax.reload();
                        }
                        if(data.success)
                        {
            ...
            {
                $.ajax({
                    url: "{{ route('cms.master-data.kategori-proklim.update') }}",
                    method: "POST",
                    data: $(this).serialize(),
                    dataType: "json",
                    beforeSend: function(){
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.master-data.kategori-proklim.edit', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
        ...
        $(document).on('click', '.delete',function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.master-data.kategori-proklim.destroy', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
            return new swal({
