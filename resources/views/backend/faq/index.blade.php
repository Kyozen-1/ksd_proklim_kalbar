...
@section('title', 'FAQ | Master Data | PROKLIM Kalimantan Barat')
@section('header', 'FAQ | Master Data')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card-box table-responsive">
                ...
                <table id="table_faq" class="table table-bordered table-bordered dt-responsive nowrap">
                    <thead>
                        <tr>
                            <th width="5%">No</th>
                            <th>Pertanyaan</th>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="form_faq" method="POST" data-parsley-validate novalidate>
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="jawaban" id="jawaban" rows="5" class="form-control" required></textarea>
                        </div>
                </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group row">
                        <label for="detail_pertanyaan" class="control-label col-md-6">Pertanyaan</label>
                        <div class="col-md-6">
                            <span id="detail_pertanyaan"></span>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="detail_jawaban" class="control-label col-md-6">Jawaban</label>
                        <div class="col-md-6">
                            <span id="detail_jawaban"></span>
                        </div>
                    </div>
                </div>
            </div><!-- /.modal-content -->
...
@section('js')
    ...
    <script>
        var dataTables = $('#table_faq').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('cms.faq.datatable') }}",
            },
            columns:[
                {
                    data: 'DT_RowIndex'
        ...
        $(document).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.faq.show', ['id' => ":id"]) }}";
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    $('#detail_pertanyaan').text(data.result.pertanyaan);
                    $('#detail_jawaban').text(data.result.jawaban);
                    $('#detail').modal('show');
                }
            });
        });
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.faq.edit', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    $('#pertanyaan').val(data.result.pertanyaan);
                    $('#jawaban').val(data.result.jawaban);
                    $('#hidden_id').val(id);
        ...
        $(document).on('click', '.delete',function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.faq.destroy', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
            return new swal({
