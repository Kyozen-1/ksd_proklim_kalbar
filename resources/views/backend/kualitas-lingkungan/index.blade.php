...
@section('title', 'Kualitas Lingkungan | PROKLIM Kalimantan Barat')
@section('header', 'Kualitas Lingkungan')
...
@section('css')
    ...
    <style>
        ...
        #table_dynamic_kualitas_lingkungan {
            vertical-align: middle;
        }

        #table_dynamic_kualitas_lingkungan th {
            text-align: center;
            vertical-align: middle;
            background-color: #f8f9fa;
        }

        #table_dynamic_kualitas_lingkungan td {
            vertical-align: middle;
        }

        #table_dynamic_kualitas_lingkungan .form-control {
            height: 36px;
        }

        #table_dynamic_kualitas_lingkungan .btn-hapus-baris {
            width: 34px;
            height: 34px;
            padding: 0;
        }
        ...
        <div class="col-md-4">
            <div class="card-box summary-card">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <div class="summary-title">Kategori Kualitas Lingkungan</div>
                        <div class="summary-value">
                            <span id="total_sektor">{{$kategoriKualitasLingkungans->count()}}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card-box summary-card">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <div class="summary-value">
                            <span id="total_data">{{$countDataKualitasLingkungan}}</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="row">
        <div class="col-12">
            <div class="card-box table-responsive">
                <div class="row mb-3">
                    <div class="col-md-7">
                        <h4 class="mt-0 header-title">Data Kualitas Lingkungan</h4>
                        <p class="text-muted mb-0">Data Kualitas Lingkungan berdasarkan Kabupaten/Kota, kategori dan tahun pendataan.</p>
                    </div>
                ...
                <div class="card border mb-3">
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="form-group mb-2">
                                    <label class="filter-label">Kategori Kualitas Lingkungan</label>
                                    <select id="filter_kategori" class="form-control select2">
                                        <option value=""> Semua Kategori </option>
                                        @foreach ($kategoriKualitasLingkungans as $kategoriKualitasLingkungan)
                                            <option value="{{$kategoriKualitasLingkungan['id']}}">{{$kategoriKualitasLingkungan['nama']}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                ...
                <table id="table_kualitas_lingkungan" class="table table-bordered table-striped dt-responsive nowrap" width="100%">
                    <thead>
                        <tr>
                            <th width="5%"> No </th>
                            <th width="10%"> Aksi </th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="createModalLabel">Tambah Data Kualitas Lingkungan</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                </div>
                <div class="modal-body">
                    <span id="form_result"></span>
                    <form class="form-horizontal" id="form_kualitas_lingkungan" method="POST" data-parsley-validate novalidate>
                        @csrf
                        ...
                        <div class="form-group">
                            <label for="kategori_kualitas_lingkungan_id" class="control-label"> Kategori Kualitas Lingkungan <span class="text-danger">*</span></label>
                            <select name="kategori_kualitas_lingkungan_id" id="kategori_kualitas_lingkungan_id" class="form-control select2" required>
                                <option value=""> Pilih Kategori Kualitas Lingkungan </option>
                                @foreach ($kategoriKualitasLingkungans as $kategoriKualitasLingkungan)
                                    <option value="{{$kategoriKualitasLingkungan['id']}}">{{$kategoriKualitasLingkungan['nama']}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <div>
                                <h5 class="mb-0"> Data Kualitas Lingkungan </h5>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-sm mb-0" id="table_dynamic_kualitas_lingkungan">
                                <thead>
                                    <tr>
                                        <th width="20%"> Tahun </th>
                                        <th width="30%"> Nilai Indeks</th>
                                        <th width="40%"> Tanggal Pendataan </th>
                                        <th width="10%"> Aksi </th>
                                    </tr>
                                </thead>
                                <tbody id="dynamic_kualitas_lingkungan_body"></tbody>
                            </table>
                        </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" form="form_kualitas_lingkungan" name="aksi_button" id="aksi_button" class="btn btn-primary waves-effect width-md waves-light"> Save </button>
                </div>
            </div>
        </div>
    </div>
...
@section('js')
    ...
    <script>
        ...
        var dataTables = $('#table_kualitas_lingkungan').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('cms.kualitas-lingkungan.datatable') }}",
                data: function (d) {
                    d.kabupaten_kota_id = $('#filter_kabupaten').val();
                    d.kategori_kualitas_lingkungan_id = $('#filter_kategori').val();
            ...
            columns: [
                ...
                {
                    data: 'kategori_kualitas_lingkungan_id',
                    name: 'kategori_kualitas_lingkungan_id'
                },
                {
                    data: 'tahun',
                    name: 'tahun',
        ...
        function tambahBaris(data = null) {
            ...
            let row = ` <tr class="dynamic-row" data-row="${rowIndex}">
                            ...
                            <td> <input type="number" name="nilai[]" class="form-control nilai-input" value="${nilai}" step="0.001" min="0" placeholder="Masukkan nilai indeks" required></td>
            ...
            $('#dynamic_kualitas_lingkungan_body') .append(row);
            $('.s2').select2();
            if (tahun) {
                $('#dynamic_kualitas_lingkungan_body tr:last') .find('.tahun-input') .val(tahun);
            }
        }

        $(document).on( 'click', '.btn-hapus-baris', function () {
            let totalRow = $('#dynamic_kualitas_lingkungan_body .dynamic-row').length;
            if (totalRow <= 1) {
                Swal.fire({
                    icon: 'warning',
                    title: 'Data tidak dapat dihapus',
                    text: 'Minimal harus terdapat satu data Kualitas Lingkungan.'
                });
                return;
        ...
        function reset() {
            $('#form_kualitas_lingkungan')[0].reset();
            $("[name='kabupaten_kota_id']") .val('') .trigger('change');
            $("[name='kategori_kualitas_lingkungan_id']") .val('') .trigger('change');
            $('#dynamic_kualitas_lingkungan_body') .empty(); rowIndex = 0;
            tambahBaris();
            $('#hidden_id').val('');
            $('#aksi').val('Save');
        }
        ...
        $('#create').click(function () {
            ...
            $('#aksi').val('Save');
            $('#createModalLabel') .text('Tambah Data Kualitas Lingkungan');
        });

        $('#form_kualitas_lingkungan').on( 'submit', function (e) {
            e.preventDefault();
            if ($('#aksi').val() == 'Save') {
                $.ajax({
                    url: "{{ route('cms.kualitas-lingkungan.store') }}",
                    method: "POST",
                    data: $(this).serialize(),
                    dataType: "json",
                    beforeSend: function () {
                    ...
                    success: function (data) {
                        ...
                        if (data.success) {
                            ...
                            $('#aksi_button') .prop('disabled', false) .text('Save');
                            $('#table_kualitas_lingkungan') .DataTable() .ajax .reload();
                            reset();
                        }
        ...
        $(document).on('click', '.save-nilai', function () {
            ...
            if (parseFloat(nilai) < 0) {
                Swal.fire({
                    ...
                    text: 'Nilai Kualitas Lingkungan tidak boleh kurang dari 0.'
                });
                nilaiInput.focus();
                return;
            }
            ...
            Swal.fire({
                ...
                text: 'Nilai Kualitas Lingkungan akan diperbarui.',
                icon: 'warning',
                ...
                if (result.value) {
                    $.ajax({
                        url: "{{ route('cms.kualitas-lingkungan.update') }}",
                        method: "POST",
                        data: {
                            _token: "{{ csrf_token() }}",
                            id: id,
