...
@section('title', 'Kegiatan | PROKLIM Kalimantan Barat')
            ...
            <div class="card-box table-responsive">
                <div class="row mb-2">
                    ...
                    <div class="col-md-6 text-right">
                        <a class="btn btn-icon waves-effect waves-light btn-primary" href="{{ route('cms.kegiatan.create') }}">
                            <i class="fas fa-plus"></i>
                        </a>
                    </div>
                </div>
                <table id="table_kegiatan" class="table table-bordered table-bordered dt-responsive nowrap">
                    <thead>
                        <tr>
                            <th width="5%">No</th>
                            <th width="10%">Aksi</th>
...
@section('js')
    ...
    <script>
        var dataTables = $('#table_kegiatan').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('cms.kegiatan.datatable') }}",
            },
            columns:[
                {
                    data: 'DT_RowIndex'
        ...
        $(document).on('click', '.delete',function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.kegiatan.destroy', ['id' => ":id"]) }}";
