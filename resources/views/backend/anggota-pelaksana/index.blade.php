...
@section('title', 'Anggota Pelaksana | PROKLIM Kalimantan Barat')
@section('header', 'Anggota Pelaksana')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card-box table-responsive">
                ...
                <table id="table_anggota_pelaksana" class="table table-bordered table-bordered dt-responsive nowrap">
                    <thead>
                        <tr>
                            <th width="5%">No</th>
                            <th width="10%">Aksi</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="form_anggota_pelaksana" method="POST" enctype="multipart/form-data" data-parsley-validate novalidate>
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <input type="file" class="dropify" id="foto" name="foto" data-height="150" data-allowed-file-extensions="png jpg jpeg webp" data-show-errors="true">
                        </div>
                </div>
...
@section('js')
    ...
    <script>
        $('#jabatan_id').select2();
        $('.dropify').dropify();
        var dataTables = $('#table_anggota_pelaksana').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('cms.anggota-pelaksana.datatable') }}",
            },
            columns:[
                {
                    data: 'DT_RowIndex'
        ...
        function reset()
        {
            $('#form_anggota_pelaksana')[0].reset();
            $("[name='jabatan_id']").val('').trigger('change');
            $('.dropify-clear').click();
        }
        ...
        $('#form_anggota_pelaksana').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('cms.anggota-pelaksana.store') }}",
                    method: "POST",
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.errors)
                        {
                            ...
                            reset();
                            $('#aksi_button').text('Save');
                            $('#table_anggota_pelaksana').DataTable().ajax.reload();
                        }
                        if(data.success)
                        {
            ...
            {
                $.ajax({
                    url: "{{ route('cms.anggota-pelaksana.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.anggota-pelaksana.edit', ['id' => ":id"]) }}";
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    let targetJabatan = data.result.jabatan;
                    let valueJabatan = $('#jabatan_id option').filter(function () {
                        return $(this).text() === targetJabatan;
                    }).val();
                    $("[name='jabatan_id']").val(valueJabatan).trigger('change');

                    var lokasiFile = data.result.foto_path;
                    var fileDropperLokasiFile = $("#foto").dropify();
        ...
        $(document).on('click', '.delete',function(){
            var id = $(this).attr('id');
            var url = "{{ route('cms.anggota-pelaksana.destroy', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
            return new swal({
